...
skeleton_path=[os.path.join('..',"generated data",i,"skeleton.png") for i in patend_ids]
vessel_mask_path=[os.path.join('..',"generated data",i,"superpixels_intersection.png") for i in patend_ids]
images_path = [(os.path.join('..','2nd data',i,'AP','AP_QCA.bmp'),os.path.join('..','2nd data',i,'AP','AP_contrast.bmp')) for i in patend_ids]
reg_images_path = [os.path.join('..','2nd data',i,'AP','AP_reg.png') for i in patend_ids]
        ...
        for j in range(visited.shape[1]):
            if(visited[i][j]):
                ...
                while queue:
                    ...
                    ### ordering ###
                    # gradient_skel[x][y] = num
                    # num += 1

                    for k in range(8):
                        tmp_x = x + d_x[k]
                        tmp_y = y + d_y[k]
                        if(not (check_range(tmp_x) and check_range(tmp_y))):continue
...
def traversal(img_512, skeleton, bool_contour):
    # skeleton : gray img
    def check_range(tmp):
        if(0<=tmp and tmp<512):
            return True
    ...
    plt.plot(X, Y) # 좌우 반전 다시 해야됨 .....
    plt.show()


for i in range(len(patend_ids)):
    #i = i+1
    print("patend_ids :",patend_ids[i])
    ...
    for i in range(len(contours)):
        contour_length= np.append(contour_length,cv2.arcLength(contours[i], False))
    ...
    tmp = skeleton.copy()
    tmp = cv2.cvtColor(tmp, cv2.COLOR_GRAY2BGR)
    for i in contours[np.argmax(contour_length)]:
        tmp[i[0][1]][i[0][0]] = [0,0,255]
        bool_contour[i[0][1]][i[0][0]] = True
   # cv2.imshow("contour + skel", tmp)

    traversal_visualize(skeleton, bool_contour)
    traversal(img_512,skeleton,bool_contour)
    print("eok")
    cv2.waitKey(0)
